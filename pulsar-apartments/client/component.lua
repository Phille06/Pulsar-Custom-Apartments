...
local function ClearApartmentPolyzones()
	for _, zone in ipairs(_apartmentPolyzones) do
		if zone then
			zone:remove()
		end
	end
...
local function BuildApartmentPolyzones()
	ClearApartmentPolyzones()
	...
	for _, aptId in ipairs(apartments) do
		...
		if apt and apt.coords then
			local zone = lib.zones.box({
				name = string.format("apt_%s", aptId),
				coords = apt.coords,
				size = vec3(apt.width or 1.0, apt.length or 1.0, ZoneHeight(apt.options)),
				rotation = apt.options and apt.options.heading or 0.0,
				debug = apt.options and apt.options.debugPoly or false,
			...
			table.insert(_apartmentPolyzones, zone)
		end
	end
end
