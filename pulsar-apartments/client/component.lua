	...
	for _, aptId in ipairs(apartments) do
		...
		if apt and apt.coords then
			local zoneId = exports.ox_target:addBoxZone({
				...
				options = {
					{
						name = string.format("apt_%s_enter", aptId),
						...
						onSelect = function()
							exports[RESOURCE]:Enter(aptId, -1)
						end,
					},
					{
					...
					},
					{
						...
						onSelect = function()
							exports["pulsar-hud"]:InputShow("Raid Apartment", "Unit Number (Owner State ID)", {
								{
									id = "unit",
									type = "number",
									options = {},
								},
							}, "Apartment:Client:DoRaid", { apartmentId = aptId })
						end,
					},
				},
			})
...
local function BuildReceptionTarget()
	if not Config.ReceptionPed or not Config.ReceptionPed.coords then
		return
	end

	RemoveZone(_receptionZone)

	local c = Config.ReceptionPed.coords
	_receptionZone = exports.ox_target:addBoxZone({
		name = "apt-reception",
